...
const ReviewsSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await AxiosPublic.get("/api/events/recent-reviews");
        const data = await response.data;
        setReviews(data);
      } catch (error) {
        console.log("Error fetching recent review", error);
      }
    };
    fetchData();
  }, []);
  return (
    <div className="py-12">
      <div>
        <h2 className="text-3xl font-bold text-center mb-8">গ্রাহকের মতামত</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
        {reviews.map((r, index) => (
          ...
          >
            ...
            {/* Review Content */}
            <div className="mt-2">
              <h3 className="text-lg font-semibold">{r.reviews.name}</h3>
              <div className="my-2">
                <ReactStars
                  count={5}
